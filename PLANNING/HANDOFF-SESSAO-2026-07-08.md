...
- **Feedback do André (14/jul):**
  ...
  - **busca por SIF no topo** (item 01) → ✅ **FEITO 14/jul** (back `49f84b7f` + front `298a346f`). Campo "Buscar pelo SIF" na Seção 1: typeahead (≥3 chars) sobre `GET /plants?search=` (rota **já existente** → **sem regenerar API GW**), preenche **Grupo+Empresa+Planta em bloco**. Backend: `findMany` passou a incluir `company{id,legalName,tradeName,groupId}` (aditivo; `Company.groupId` é obrigatório → todo SIF resolve p/ 1 grupo). `hydratingRef` evita o reset-da-planta-ao-trocar-grupo, e só é armado se o grupo muda de fato. Fluxo grupo→planta segue intacto. Complementa o `#8` (exibe o SIF selecionado). ⚠️ front depende do include do back — se subir antes, a busca não resolve o grupo até o back deployar.
